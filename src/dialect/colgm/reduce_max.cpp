...
void reduce_max::build(mlir::OpBuilder& builder, mlir::OperationState& state,
...
reduce_max reduce_max::create(mlir::OpBuilder& builder, mlir::Location loc,
                               mlir::Value input, mlir::ArrayRef<i64> axes) {
    mlir::OperationState state(loc, getOperationName());
    build(builder, state, input, axes);
    return llvm::cast<reduce_max>(builder.create(state));
}

mlir::ParseResult reduce_max::parse(mlir::OpAsmParser& parser,
...
void reduce_max::print(mlir::OpAsmPrinter& p) {
    p << " " << get_input()
      << " {axes = " << get_axes() << "}"
      << " : " << get_input().getType()
...
mlir::LogicalResult reduce_max::verify() {
